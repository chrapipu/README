...
<img width="837" height="802" alt="pLJBJi9G4DqtwJzCkj94_00IZ04aCRwYDMpDjRsQncjBxhqGtVbQcEZUdMxy07pW0mH_OUuVERUbO0YojXlAd3CpP-PCkgmqaxfvB5n7Hrfmm0SSc7FiOGTVSO3Fs3ONzDupjzYtYBamjr00lCCslfWpSudlsCKsc2iAV59U6-OsMCmEkLo0CWkEU1n2HJ2bk5g0ZLD_UttUTLo71JgHC6" src="https://github.com/user-attachments/assets/912da553-803b-480c-a36c-fe145f7f923a" />
...
Безопасность данных. Для обеспечения безопасности пользовательских данных реализован ряд мер. Пароли в базе данных не хранятся в открытом виде, а хэшируются с использованием алгоритма BCryptPasswordEncoder, встроенного в Spring Security. Это гарантирует, что даже при компрометации базы злоумышленник не сможет восстановить исходные пароли. При регистрации и смене пароля проверено, что данные проходят через хэширование:
...
Для разработки настроен CORS, разрешающий запросы с фронтенда:
...
Архитектура разделена: JWT используется только для идентификации и передачи «лёгких» данных (id, имя, роли, дата рождения), а полный профиль пользователя загружается отдельным API‑запросом.
Для дополнительной защиты на уровне методов включена аннотация @EnableMethodSecurity, что позволяет использовать декларативные проверки доступа. Например, доступ к методам, предназначенным только для родителей, ограничен с помощью:
...
Такой подход обеспечивает комплексную защиту: пароли надёжно хранятся, токены защищены подписью, фронтенд и бэкенд взаимодействуют безопасно, а доступ к данным строго контролируется ролями.
Рефакторинг. В процессе разработки был проведён рефакторинг кода с целью повышения читаемости и поддерживаемости. DTO‑объекты приведены к «чистому» виду: в них оставлены только необходимые для фронтенда поля (строковые представления ролей, avatarUrl и др.), что исключает избыточность и упрощает сериализацию.
На стороне бэкенда реализован подход CQRS: для каждой операции выделен отдельный хэндлер. Так, получение профиля пользователя обрабатывается через GetUserProfileHandler, а обновление – через UpdateUserProfileHandler. Это обеспечивает чёткое разделение команд и запросов, упрощает тестирование и расширение функционала.
На фронтенде синхронизация с бэкендом реализована через контекст UserContext. Он отвечает за хранение и обновление состояния профиля пользователя, автоматически подтягивает данные из токена и API, а также обеспечивает доступ к актуальной информации во всех компонентах приложения. Такой подход устраняет дублирование логики и гарантирует согласованность данных между клиентом и сервером.
Пример:

// Бэкенд: отдельные хэндлеры
...
// Фронтенд: контекст пользователя
...
Программное средство FR Analytics предназначено для реализации ана-литического решения в сфере розничных продаж food‑ритейла. Архитектура системы построена по трёхуровневой модели: frontend – пользовательский интерфейс (React, Vite, TypeScript/JavaScript); backend – бизнес‑логика и API (Java, Spring Boot); database – хранение данных (MySQL).
Система поддерживает две роли:
	Менеджер – загружает данные в формате Excel, выполняет анализ, формирует таблицы и дашборды.
	Директор – просматривает результаты анализа и выгружает отчёты в Excel или PDF.
Регистрация пользователей отсутствует: в базе данных заранее опреде-лены два пользователя (менеджер и директор).
Требования к окружению. Операционная система: Windows, Linux или macOS. Установленное ПО: JDK 21+; Maven 3.9+; Node.js 18+ (npm); MySQL 8+. Свободные порты: 3306 – база данных; 8080 – backend; 5173 – frontend.
Для работы системы необходимо создать базу данных и пользователей.
init.sql:

CREATE DATABASE IF NOT EXISTS fr_analytics CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

CREATE USER IF NOT EXISTS 'fr_user'@'localhost' IDENTIFIED BY 'StrongPassword123!';
GRANT ALL PRIVILEGES ON fr_analytics.* TO 'fr_user'@'localhost';
FLUSH PRIVILEGES;
...
Конфигурация backend application.properties:

spring.application.name=fr-analytics
server.port=8080
spring.jpa.properties.hibernate.dialect=org.hibernate.dialect.MySQL8Dialect
spring.jpa.hibernate.ddl-auto=update
spring.datasource.url=jdbc:mysql://localhost:3306/fr_analytics
spring.datasource.username=fr_user
spring.datasource.password=StrongPassword123!
spring.flyway.baseline-on-migrate=true

1 Запуск MySQL и подготовка схемы.
	Убедиться, что MySQL запущен.
	Выполнить скрипт init.sql.
2 Сборка и запуск backend.

cd fr-analytics-backend
mvn clean package
java -jar target/fr-analytics-0.0.1-SNAPSHOT.jar

Доступ: http://localhost:8080.

3 Сборка и запуск frontend.

cd fr-analytics-frontend
npm install
npm run build
npm run preview
...
Проверка работы:
	Менеджер загружает Excel файл через frontend → backend проверяет корректность и сохраняет данные в MySQL.
	Backend формирует таблицы и дашборды.
	Frontend отображает результаты анализа.
	Директор получает доступ к отчётам и может скачать их в формате Excel или PDF.
